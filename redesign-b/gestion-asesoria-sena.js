...
rowsBox?.addEventListener('click',async e=>{const b=e.target.closest('button[data-receipt]');if(!b)return;statusBox.textContent='Abriendo comprobante…';try{const r=await api(`/admin/solicitudes/${b.dataset.receipt}/comprobante`);const blob=await r.blob();const url=URL.createObjectURL(blob);window.open(url,'_blank','noopener');setTimeout(()=>URL.revokeObjectURL(url),60000);statusBox.textContent='Comprobante abierto.';}catch(err){statusBox.textContent=err.message;}});
